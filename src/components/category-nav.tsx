    ...
    <>
      {tiles.map((c) => (
        <Link
          ...
          className={`flex shrink-0 items-center gap-2 rounded-full px-3 py-1 text-xs font-medium transition-colors hover:bg-slate-100 ${
            activeSlug === c.slug ? "bg-brand/10 text-brand" : "text-foreground"
          }`}
        >
          <div className="relative size-7 shrink-0 overflow-hidden rounded-full bg-slate-100">
            <Image
    ...
    <nav className="border-b border-border-subtle bg-background">
      <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-4 px-4">
        <div className="flex items-center gap-1 overflow-x-auto py-2 scrollbar-none">
          <Suspense fallback={
            tiles.map((c) => (
              <div key={c.slug} className="flex shrink-0 items-center gap-2 rounded-full px-3 py-1 text-xs font-medium text-foreground">
