          ...
          <>
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {lines.map(({ product, qty }) => (
                <div key={product.id} className="flex gap-3 rounded-card border border-border-subtle p-2">
                  ...
                  <div className="flex flex-1 flex-col">
                    ...
                    <div className="mt-auto flex items-center gap-2">
                      <div className="flex items-center rounded-md border border-border-subtle">
                        <button onClick={() => setQty(product.id, qty - 1)} className="grid size-7 place-items-center hover:bg-muted" aria-label="Хасах">
                          <Minus className="size-3.5" />
                        </button>
                        <span className="w-8 text-center text-sm">{qty}</span>
                        <button onClick={() => setQty(product.id, qty + 1)} className="grid size-7 place-items-center hover:bg-muted" aria-label="Нэмэх">
                          <Plus className="size-3.5" />
                        </button>
                      </div>
                      <button onClick={() => remove(product.id)} className="ml-auto p-1 text-muted-foreground hover:text-brand" aria-label="Устгах">
