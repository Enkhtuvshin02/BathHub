          ...
          <div className="overflow-hidden rounded-card border border-border-subtle">
            ...
            <ul>
              {lines.map(({ product, qty }) => (
                <li key={product.id} className="flex flex-col gap-3 border-b border-border-subtle p-4 last:border-0 sm:flex-row sm:items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center rounded-lg border border-border-subtle">
                      <button onClick={() => setQty(product.id, qty - 1)} className="grid size-9 place-items-center hover:bg-muted" aria-label="Хасах">
                        <Minus className="size-4" />
                      </button>
                      <span className="w-10 text-center text-sm font-medium">{qty}</span>
                      <button onClick={() => setQty(product.id, qty + 1)} className="grid size-9 place-items-center hover:bg-muted" aria-label="Нэмэх">
                    ...
                    <button onClick={() => remove(product.id)} aria-label="Устгах" className="grid size-9 place-items-center rounded-lg border border-border-subtle text-muted-foreground hover:text-sale">
                      <X className="size-4" />
                    </button>
                  </div>
                </li>
